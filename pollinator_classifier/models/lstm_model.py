"""Unidirectional LSTM classifier for micro-Doppler pollinator signals."""
...
class LSTMModel(BaseModel):
    ...
    def _build(self, input_shape: tuple) -> None:
        ...
        x = tf.keras.layers.LSTM(LSTM_UNITS_1, return_sequences=True)(inputs)
        x = tf.keras.layers.Dropout(DROPOUT_RATE)(x)
        x = tf.keras.layers.LSTM(LSTM_UNITS_2)(x)
        x = tf.keras.layers.Dropout(DROPOUT_RATE)(x)
        outputs = tf.keras.layers.Dense(NUM_CLASSES, activation="softmax")(x)

        self._model = tf.keras.Model(inputs, outputs, name="lstm")
        self._model.compile(
            optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
            loss="categorical_crossentropy",
            metrics=["accuracy"],
        )
        logger.debug("LSTM model built: %s", input_shape)
